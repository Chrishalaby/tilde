...
describe('store with a localStorage stub', () => {
  it('round-trips settings', () => {
    ...
    expect(stub.getItem('tilde.settings')).toBeTypeOf('string');
  ...
  it('falls back to defaults when the stored value is corrupt', () => {
    const stub = install();
    stub.setItem('tilde.settings', '{not json');
    expect(loadSettings()).toEqual(DEFAULT_SETTINGS);
    stub.setItem('tilde.settings', '"nope"');
    expect(loadSettings()).toEqual(DEFAULT_SETTINGS);
  ...
  it('keeps known fields when unknown ones are stored', () => {
    const stub = install();
    stub.setItem('tilde.settings', JSON.stringify({ cellW: 9, junk: true }));
